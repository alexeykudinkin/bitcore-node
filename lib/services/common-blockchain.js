...
CommonBlockchainService.prototype.setupRoutes = function (app, express) {
  ...
  function extractTransactions(txIds) {
  ...
  app.get('/addresses/summary/:addrs', function (req, res) {
    ...
    if (!addrs || addrs.length === 0)
      res.status(404).send('Sorry, we cannot find that addresses!\n');
  ...
  app.get('/addresses/transactions/:addrs', function (req, res) {
    ...
    self.addresses.transactions(addrs, req.query.height, function (err, result) {
      if (!err)
        res.json(result);
      else
        res.status(500).send('Sorry, something blew up!\n' + err);
  ...
  app.get('/transactions/get/:txIds', function (req, res) {
    var txIds = extractTransactions(req.params.txIds);

    if (!txIds || txIds.length === 0)
      res.status(404).send('Sorry, we cannot find that transactions!\n');
  ...
  app.post('/transactions/propagate', function (req, res) {
    var txs = extractTransactions(req.body.txs);

    if (!txs || txs.length === 0)
      res.status(400).send('No TX hexs found!\n' + req);

    self.transactions.propagate(req.body.txs, function (err, result) {
      if (!err)
        res.json(result);
      else
        res.status(500).send('Sorry, something blew up!\n' + err);
...
Transactions.prototype.propagate = function (txs, callback) {
  ...
  async.map(
    txs,
    function (tx, reduce) {
      self.parent.node.services.db.sendTransaction(tx, /* queryMempool = */ false, reduce)
    },
    callback
  );
};
...
Blocks.prototype.get = function (hashes, callback) {
  ...
  async.map(
    ...
    function (hash, reduce) {
      self.parent.node.services.db.getBlock(hash, /* queryMempool = */ false, function (err, block) {
        if (!err) {
          reduce(err, {
            blockId:  block.id,
